...
T["await()"]["errors when called outside coroutine"] = function()
  ...
  eq(
    result[2]:find "Ensure the surrounding function is wrapped in a `make_spawn` or a `make_async`"
      ~= nil,
    true
  )
end
...
T["await()"]["works inside async function"] = function()
  local async_fn = M.make_async(function()
    local inner_promise = M.from_executor(function(resolve)
      resolve(42)
    end)
...
T["await()"]["rejects when awaited promise throws"] = function()
  local bad_async_fn = M.make_async(function()
    error "boom"
  end)
  local outer_async_fn = M.make_async(function()
    local bad_promise = bad_async_fn()
    return M.await(bad_promise)
  end)

  local err = nil
  local done = false
  local outer_promise = outer_async_fn()
  outer_promise(function() end, function(e)
    err = e
    done = true
  end)
  vim.wait(10, function()
...
T["await()"]["propagates non-string errors"] = function()
  ...
  eq(
    { done = done, ok = result[1], code = result[2] and result[2].code or nil },
    { done = true, ok = false, code = 42 }
  )
end

T["await()"]["catches sync throw from from_executor promise"] = function()
  local bad_promise = M.from_executor(function()
    error "boom"
  end)
  local outer_async_fn = M.make_async(function()
    local ok, err = pcall(M.await, bad_promise)
...
T["from_executor()"]["passes reject callback"] = function()
  local err = nil
  local done = false
  local promise = M.from_executor(function(_, reject)
    reject "nope"
  end)
...
T["from_executor()"]["rejects when executor throws"] = function()
  local err = nil
  local done = false
  local promise = M.from_executor(function()
    error "boom"
  end)
  promise(function() end, function(e)
    err = e
...
T["make_async()"]["resolves with return value"] = function()
  ...
  local add_promise = add_async_fn(3, 4)
  add_promise(function(v)
    value = v
    done = true
  end)
...
T["make_async()"]["rejects when fn throws"] = function()
  ...
  local bad_promise = bad_async_fn()
  bad_promise(function() end, function(e)
    err = e
    done = true
  end)
  eq({ done = done, boom = err and err:find "boom" ~= nil or false }, { done = true, boom = true })
...
T["make_async()"]["rethrows when no reject passed"] = function()
  ...
  local bad_promise = bad_async_fn()
  local result = { pcall(bad_promise, function() end) }
  eq(result[1], false)
  eq(result[2]:find "boom" ~= nil, true)
end
...
T["throttled_iterator()"]["iterates over all values"] = function()
  ...
  local promise = M.throttled_iterator(function()
    return function(_, n)
      if n < 3 then
        return n + 1
      end
...
T["throttled_iterator()"]["rejects when on_iteration throws after yield"] = function()
  ...
  eq(
    { done = done, boom = err and err:find "iter boom" ~= nil or false },
    { done = true, boom = true }
  )
end
...
T["integration"]["awaits async and callback promises inside make_spawn"] = function()
  ...
  local spawn = M.make_spawn(function()
    local add_promise = add_async_fn(3, 4)
    local sum = M.await(add_promise)
    local double_promise = double_async_fn(sum)
    local doubled = M.await(double_promise)
